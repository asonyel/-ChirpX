...
type Clip={id:string;post_id:string;owner_id:string;storage_path:string;caption:string;created_at:string;mux_playback_id?:string|null;processing_status?:string;moderation_status?:string;url?:string};
...
export default function ClipsPage(){
 ...
 async function load(){
   ...
   const hydrated=await Promise.all(rows.map(async c=>{if(c.mux_playback_id)return c;const {data:s}=await supabase.storage.from(MEDIA_BUCKET).createSignedUrl(c.storage_path,3600);return {...c,url:s?.signedUrl};})); setClips(hydrated);
 }
 ...
 async function upload(){
   ...
   try{
     ...
     setMessage('Uploading securely…'); await resumableUpload(path,file); setMessage('Creating media pipeline…');
     const {data:clip,error:clipError}=await supabase.from('clips').insert({post_id:post.id,owner_id:user.id,storage_path:path,caption,status:'uploaded',processing_status:'uploaded',moderation_status:'pending'}).select('id').single(); if(clipError||!clip)throw clipError??new Error('Could not create Clip record.');
     setMessage('Submitting adaptive-video processing…');
     const dispatch=await supabase.functions.invoke('media-dispatch',{body:{clip_id:clip.id}});
     if(dispatch.error) setMessage('Upload is safe. Media processing is waiting for provider activation.'); else setMessage('Upload complete. Chirpx is transcoding, captioning and moderating your Clip.');
     setCaption(''); setFile(null); setProgress(100); await load();
...
function ClipCard({clip,liked,onLike,onReply,onRechirp}:{clip:Clip;liked:boolean;onLike:()=>void;onReply:()=>void;onRechirp:()=>void}){
 const videoRef=useRef<any>(null); const started=useRef<number>(0); const reported=useRef(false); const completed=useRef(false);
 ...
 return <article className="card" style={{overflow:'hidden',scrollSnapAlign:'start',minHeight:'78vh',position:'relative',background:'#000'}}>
   {clip.mux_playback_id?<MuxPlayer ref={videoRef} playbackId={clip.mux_playback_id} streamType="on-demand" autoPlay muted loop playsInline onTimeUpdate={timeUpdate} metadata={{video_id:clip.id,video_title:clip.caption||'Chirpx Clip'}} style={{width:'100%',height:'72vh',background:'#000'}}/>:clip.url?<video ref={videoRef} src={clip.url} playsInline loop muted preload="metadata" onTimeUpdate={timeUpdate} style={{width:'100%',height:'72vh',background:'#000',objectFit:'contain'}}/>:<div style={{height:'72vh',display:'grid',placeItems:'center'}} className="muted">Processing Clip…</div>}
   <div style={{padding:14,background:'#0d1117'}}><p>{clip.caption}</p><div className="muted" style={{fontSize:12,marginBottom:8}}>{clip.processing_status==='ready'?'Adaptive stream ready':'Processing'} · moderation {clip.moderation_status??'pending'}</div><div className="actions"><button onClick={onLike}>{liked?'♥ Liked':'♡ Like'}</button><button onClick={onReply}>♡ Reply</button><button onClick={onRechirp}>↻ Rechirp</button><button className="tab" onClick={()=>navigator.share?.({title:'Chirpx Clip',text:clip.caption,url:location.href})}>↗ Share</button></div></div>
 </article>;
}
